...
function createAndShowNotification(message) {
    return new Promise((resolve) => {
        // Adiciona os estilos para notificações se ainda não estiverem presentes
        ...
        notification.style.bottom = `${20 + (notificationCount - 1) * 70}px`; // Posiciona a notificação
        notification.style.right = "20px"; // Define a posição à direita
        ...
        const closeButton = document.getElementById(`close-btn-${notificationCount}`);
        closeButton.onclick = function() {
            notification.style.right = "-320px"; // Anima a saída da notificação
            setTimeout(() => {
                notification.remove(); // Remove a notificação após a animação
                notificationCount--; // Decrementa o contador
                resolve(); // Resolve a promise
            }, 500); // Tempo de espera para animação
        };

        // Anima a entrada da notificação
        setTimeout(() => {
            notification.style.right = "20px"; // Move a notificação para a posição visível
        }, 10);

        // Remove a notificação após 5 segundos se não for fechada
        setTimeout(() => {
            if (notification) {
                notification.style.right = "-320px"; // Anima a saída da notificação
                setTimeout(() => {
        ...
        }, 5000); // Tempo antes de remover a notificação automaticamente
    });
}
